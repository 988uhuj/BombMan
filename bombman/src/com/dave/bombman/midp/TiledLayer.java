...
public class TiledLayer extends Layer {
	 ...
	 */
	public TiledLayer( int rows, int columns, Bitmap image, int tileWidth,
			int tileHeight) {
	 ...
	 */
	public int createAnimatedTile(int staticTileIndex) {//这个名为产生动态图片，其实根本没用
		// checks static tile
		...
		if (visible) {
			...
			for (int row = 0; row < cellMatrix.length; row++) {
				...
				for (int column = 0; column < totalCols; column++) {
					...
					Rect src = new Rect(tileSetX[tileIndex],
							tileSetY[tileIndex], tileSetX[tileIndex]
									+ cellWidth, tileSetY[tileIndex]
									+ cellHeight);
					Rect dst = new Rect(this.x + column * cellWidth, this.y
							+ row * cellHeight, this.x + column * cellWidth
							+ cellWidth, this.y + row * cellHeight
							+ cellHeight);
					c.drawBitmap(sourceImage, src, dst, new Paint());
				}
			}
