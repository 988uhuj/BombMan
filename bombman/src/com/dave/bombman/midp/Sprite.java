...
public class Sprite extends Layer
{
    ...
    public final void paint(Canvas c) {
		...
		if (visible) {
			...
			Rect dst = new Rect(this.x, this.y, this.x + srcFrameWidth, this.y
					+ srcFrameHeight);//显示的矩形位置
			...
			c.drawBitmap(bmp, null, dst, new Paint());
		}
     ...
     *
     */
    public void setFrameSequence(int sequence[]) {//显示图片的哪几帧
    ...
    public int getTransform() {
    	return t_currentTransformation;
    }
	...
	private void initializeFrames(Bitmap image, int fWidth, int fHeight,
			boolean maintainCurFrame) {//指定图片，指定一张图片被分为多少帧，每一帧的坐标
	...
	private static boolean doPixelCollision(int image1XOffset,
		...
		if (0x0 != (transform2 & INVERTED_AXES)) {
			...
			image2.getPixels(argbData1, 0, height, // scanlength = height
					image1XOffset, image1YOffset, height, width);
